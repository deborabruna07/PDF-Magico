      ...
      <div className="relative shadow-[0_20px_50px_-12px_rgba(244,114,182,0.15)] rounded-2xl mx-auto transition-all duration-300" style={{ width: canvasSize.width, height: canvasSize.height }}>
        ...
        <div 
          ...
          onClick={(e) => {
            ...
            // Trava para criar texto apenas com a ferramenta certa
        ...
        >
          ...
          <div className={cn("absolute inset-0 z-10", (drawing || activeTool === 'draw' || activeTool === 'sign') && "pointer-events-none")}>
            {annotations.filter((a): a is ImageAnnotation | TextAnnotation => (a.type === 'image' || a.type === 'text') && a.pageIndex === pageIndex).map((ann) => {
              ...
              return (
                ...
                >
                  ...
                  {ann.type === 'image' ? (
                    <div className={cn("relative block rounded-xl transition-all pointer-events-auto", selectedElement === ann.id ? "border-2 border-pink-400 border-dashed bg-pink-400/10" : "border-2 border-transparent hover:border-pink-300/40")} 
                      ...
                      onMouseDown={(e) => {
                        if (activeTool === 'draw' || activeTool === 'sign') {
                          e.preventDefault(); // ✨ Isso impede que o navegador selecione o texto enquanto você risca ✨
                          setDrawing(true);
                        }
                      }}
                    ...
                    <div className="pointer-events-auto" onMouseDown={(e) => { if (activeTool === 'select' && editingText !== ann.id) { e.stopPropagation(); setDragInfo({ id: ann.id, offsetX: getRelativePos(e).x - ann.x, offsetY: getRelativePos(e).y - ann.y }); } }}>
                      {editingText === ann.id ? (
          ...
          {/* SVG de desenho no FINAL com z-index alto para ficar por cima de tudo */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-[100]">
            ...
            {drawing && currentPath.length > 1 && (
              <polyline 
                ...
                strokeWidth={activeTool === 'sign' ? 2 : drawWidth} 
                strokeLinecap="round" 
