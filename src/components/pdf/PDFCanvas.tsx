...
export function PDFCanvas({
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (rotatingId) {
      ...
      if (ann && 'x' in ann) {
        ...
        const snap = 5;
        if (Math.abs(angle % 90) < snap || Math.abs(angle % 90) > 90 - snap) angle = Math.round(angle / 90) * 90;
        onUpdateAnnotation(rotatingId, { rotation: angle }, true);
      }
      return;
    }
    if (drawing) setCurrentPath((prev) => [...prev, pos]);
    else if (dragInfo) onUpdateAnnotation(dragInfo.id, { x: pos.x - dragInfo.offsetX, y: pos.y - dragInfo.offsetY }, true);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    setDrawing(false); setCurrentPath([]); setDragInfo(null); setRotatingId(null);
  ...
  return (
    <div className="pdf-canvas-container flex-1 overflow-auto scrollbar-thin bg-rose-50/40 p-10 transition-colors duration-500 relative">
      <div className="relative shadow-[0_20px_50px_-12px_rgba(244,114,182,0.15)] rounded-2xl mx-auto transition-all duration-300" style={{ width: canvasSize.width, height: canvasSize.height }}>
        ...
        <div ref={overlayRef} className={cn('absolute inset-0', (activeTool === 'draw' || activeTool === 'sign') && 'cursor-crosshair', (activeTool === 'text' || activeTool === 'edit-text') && 'cursor-text', activeTool === 'image' && 'cursor-cell', rotatingId && 'cursor-grabbing')} 
          onClick={(e) => {
            if (editingText) {
                const currentAnn = annotations.find(a => a.id === editingText) as TextAnnotation;
                if (currentAnn && (!currentAnn.text || !currentAnn.text.trim()) && !currentAnn.backgroundColor) onRemoveAnnotation(editingText);
                setEditingText(null); return; 
            }
            if (selectedElement) { setSelectedElement(null); return; }
            if (activeTool === 'text' || activeTool === 'edit-text') {
                const pos = getRelativePos(e);
                const id = crypto.randomUUID();
                onAddAnnotation({ id, type: 'text', pageIndex, x: pos.x, y: pos.y, text: '', fontSize: 16, color: '#4c1d95', backgroundColor: activeTool === 'edit-text' ? '#ffffff' : undefined, fontFamily: 'Helvetica', rotation: 0, fontWeight: 'normal' });
                setEditingText(id);
        ...
        >
          ...
          {annotations.filter((a): a is ImageAnnotation | TextAnnotation => (a.type === 'image' || a.type === 'text') && a.pageIndex === pageIndex).map((ann) => {
            ...
            return (
              <div key={ann.id} className="absolute" style={{ left: ann.x * scale, top: ann.y * scale, zIndex: isSelected ? 50 : 10, transform: `rotate(${ann.rotation || 0}deg)`, transformOrigin: 'center center' }} >
                {isSelected && (
                  <div className="absolute left-full ml-4 top-0 flex items-center gap-2 bg-white shadow-[0_8px_30px_rgb(244,114,182,0.12)] border border-pink-100 p-1.5 rounded-2xl z-50 pointer-events-auto" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                    {ann.type === 'text' && (
                      <div className="flex items-center gap-2 border-r border-pink-100 pr-2 mr-1">
                        ...
                        <select value={ann.fontFamily || 'Helvetica'} onChange={(e) => onUpdateAnnotation(ann.id, { fontFamily: e.target.value })} 
                          className="h-8 text-xs border-2 border-pink-100 rounded-xl bg-white text-pink-950 font-bold px-2 outline-none focus:ring-2 focus:ring-pink-300 transition-all cursor-pointer appearance-none" style={{ colorScheme: 'light' }}>
                          <optgroup label="☆ PADRÃO PDF ☆" className="font-black" style={{ backgroundColor: '#fff', color: '#831843' }}>
                              <option value="Helvetica" className="font-normal" style={{ backgroundColor: '#fff', color: '#000' }}>Helvetica / Arial</option>
                              <option value="Times New Roman" className="font-normal" style={{ backgroundColor: '#fff', color: '#000' }}>Times New Roman</option>
                              <option value="Courier" className="font-normal" style={{ backgroundColor: '#fff', color: '#000' }}>Courier</option>
                          </optgroup>
                          <optgroup label="☆ GOOGLE FONTS ☆" className="font-black" style={{ backgroundColor: '#fff', color: '#831843' }}>
                              <option value="Roboto" className="font-normal" style={{ backgroundColor: '#fff', color: '#000' }}>Roboto</option>
                              <option value="Montserrat" className="font-normal" style={{ backgroundColor: '#fff', color: '#000' }}>Montserrat</option>
                              <option value="Oswald" className="font-normal" style={{ backgroundColor: '#fff', color: '#000' }}>Oswald (Impact)</option>
                              <option value="Comic Neue" className="font-normal" style={{ backgroundColor: '#fff', color: '#000' }}>Comic Neue</option>
                          </optgroup>
                        </select>
                      </div>
                    )}
